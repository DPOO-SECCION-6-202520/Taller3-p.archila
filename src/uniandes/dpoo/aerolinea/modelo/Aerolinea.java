...
public class Aerolinea
{
    ...
    public Vuelo getVuelo( String codigoRuta, String fechaVuelo )
    {
    	...
    	{
    		Vuelo vueloIndex = vuelos.get(i);
    		String fecha = vueloIndex.getFecha();
    		if(fecha.equals(fechaVuelo)) 
    		{
    			Ruta ruta = vueloIndex.getRuta();
    ...
    public Collection<Tiquete> getTiquetes( )
    {
    	...
    	for(int i = 0; i < vuelos.size(); i ++) 
    	{
    		...
    		ArrayList<Tiquete> listaTiquetes = vuelo.getTiquetes();
    		for(int j = 0; j < listaTiquetes.size(); j++) 
    		{
    			tiquetes.add(listaTiquetes.get(i));
    		}
    ...
    {
    	String archivo1 = "CentralPersistencia.JSON";
    	String archivo2 = "CentralPersistencia.PLAIN";
    	if( archivo1.equals( tipoArchivo ) )
    		new PersistenciaAerolineaJson( ).cargarAerolinea(archivo, null);
    	else if(archivo2.equals(tipoArchivo)) 
    	{
    		new PersistenciaAerolineaPlaintext().cargarAerolinea(archivo, null);;
    	}
    	else 
    	{
    ...
    public void salvarAerolinea( String archivo, String tipoArchivo ) throws TipoInvalidoException, IOException
    {
    	...
    	String archivo2 = "CentralPersistencia.PLAIN";
    	if( archivo1.equals( tipoArchivo ) )
    		new PersistenciaAerolineaJson( ).salvarAerolinea(archivo, null);
    	else if(archivo2.equals(tipoArchivo)) 
    	{
    		new PersistenciaAerolineaPlaintext().salvarAerolinea(archivo, null);;
    	}
    	else 
    	{
    		throw new TipoInvalidoException(tipoArchivo);
    ...
    public void programarVuelo( String fecha, String codigoRuta, String nombreAvion ) throws Exception
    {
        ...
        Avion avion = null;
        int i = 0;
        while(i < aviones.size() && avion == null) 
        {
        	Avion avionPos = aviones.get(i);
        	String nombre = avionPos.getNombre();
    ...
    public int venderTiquetes( String identificadorCliente, String fecha, String codigoRuta, int cantidad ) throws VueloSobrevendidoException, Exception
    {
    	Ruta ruta = rutas.get(codigoRuta);
    	 Vuelo vuelo = null;
         int i = 0;
         while(i < vuelos.size() && vuelo == null) 
         {
         	Vuelo vueloPos = vuelos.get(i);
         	Ruta rutaPrueba = vueloPos.getRuta();
         	if(rutaPrueba.equals(ruta)) 
         	{
         		vuelo = vueloPos;
         	}
         	i += 1;
         ...
         ArrayList<Tiquete> tiquetes = vuelo.getTiquetes();
         int j = 0;
         while(j < tiquetes.size() && cantidad > 0) 
         {
        	 Tiquete tiquete = tiquetes.get(j);
        	 if(tiquete.esUsado() == false) 
        	 {
    ...
    public void registrarVueloRealizado( String fecha, String codigoRuta )
    {
        ...
        int i = 0;
        while(i < vuelos.size() && eliminado == false) 
        {
        	Vuelo vueloPos = vuelos.get(i);
        	Ruta rutaPrueba = vueloPos.getRuta();
        	String fechaPrueba = vueloPos.getFecha();
        	if(rutaPrueba.equals(ruta) && fechaPrueba.equals(fecha)) 
        	{
        		vuelos.remove(i);
        		eliminado = true;
        	}
    ...
    public int consultarSaldoPendienteCliente( String identificadorCliente )
    {
        ...
        ArrayList<Tiquete> listaTiquetes = cliente.tiquetesSinUsar;
        int precioTotal = 0;
        for(int i = 0; i < listaTiquetes.size(); i++) 
        {
        	Tiquete tiquete = listaTiquetes.get(i);
        	Vuelo vuelo = tiquete.getVuelo();
        	precioTotal = precioTotal + calcularTarifa(vuelo, cliente);
        }
        return 0;
    }
